...
3. Chuẩn bị tài khoản và API key ShopAIKey
4. Clone repo: git clone <repo-url>
5. Chạy: cd legal_multiagent && uv sync
6. Copy .env.example thành .env và thêm SHOPAIKEY_API_KEY
...
- [ ] Internet ổn định (cần gọi ShopAIKey API)
...
- ShopAIKey: OpenAI-compatible API endpoint `https://api.shopaikey.com/v1`
...
```bash
# Check .env file
cat .env | grep SHOPAIKEY_API_KEY

# Test API key
curl https://api.shopaikey.com/v1/models \
...
- Có thể đổi model qua biến `SHOPAIKEY_MODEL`; mặc định project dùng `gpt-5.4-mini`
